...
public partial class TileSetter : Node
{
	...
	public static Tiles tileFromArrays(float heightValue, float heatValue, float moistureValue) {
		if (heightValue < .5) {
			...
			else if (heatValue > .9) { return Tiles.Badlands; } 
			else { 
				if (moistureValue < heightValue + (heightValue - .5) * 5) { return Tiles.Beach; }
				else if (heatValue < .12) { return Tiles.Ice; }
				else { return Tiles.Water; }
			}
			...
			else if (heatValue > .85) { return Tiles.Badlands; } 
			else {
